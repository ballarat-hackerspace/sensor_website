...
void _dhtSampler() {
    if ((Time.now() - DHT_PublishTime) > DHT_SAMPLE_INTERVAL) {
        ...
        if (!DHT.acquiring()) {
            ...
            if (result == DHTLIB_OK) {
                char t[150];
	            sprintf(t, "{\"temperature\":\"%2.2f\",\"humidity\":\"%2.2f\",\"dewPoint\":\"%2.2f\",\"coreid\":\"%s\",\"timestamp\":\"%d\"}",
	                DHT.getCelsius(), DHT.getHumidity(), DHT.getDewPoint(), Particle.deviceID().c_str(), Time.now());
